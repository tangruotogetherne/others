...
pixel = 0.186
x_arcsec = 10
r = np.sqrt((x_arcsec/pixel)**2/(2*np.pi))
photoz = 30.

hdu_seg = fits.open('/home/tac/Documents/SBlimit/new/D1.R.seg03.fits')               # Sementation image
...
M_lim = -2.5*np.log10(sdev_0/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_0 =', M_lim, '\n')
f = open('SBlimit_print_new.txt','a')
f.write('\nlength of flux0 ')
...
M_lim = -2.5*np.log10(sdev_1/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_1 =', M_lim, '\n')
f.write('\nlength of flux1 ')
...
M_lim = -2.5*np.log10(sdev_2/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_2 =', M_lim, '\n')
f.write('\nlength of flux2 ')
...
M_lim = -2.5*np.log10(sdev_3/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_3 =', M_lim, '\n')
f.write('\nlength of flux3 ')
...
M_lim = -2.5*np.log10(sdev_4/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_4 =', M_lim, '\n')
f.write('\nlength of flux4 ')
...
M_lim = -2.5*np.log10(sdev_5/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_5 =', M_lim, '\n')
f.write('\nlength of flux5 ')
...
M_lim = -2.5*np.log10(sdev_6/(2*np.pi*r**2*pixel**2)) + photoz
print('M_lim_6 =', M_lim, '\n')
f.write('\nlength of flux6 ')
...
# Extract flux_4 as the last result whose SBlimit and sigma all became an constant respectively
...
# <------ Step 3. Calculate background variation ------>


# Show the distribution of background with histogram statistics
# Using probability density distribution function of norm for fitting

matplotlib.rc('xtick', labelsize=20)
matplotlib.rc('ytick', labelsize=20)

xgrid = np.arange(-450,150,20) 
xcenter = (xgrid[1:]+xgrid[:-1])/2 
hx, xedge = np.histogram(flux_4, xgrid)

mean_4 = np.mean(flux_4) 
sigma_4 = np.std(flux_4) 
flux_fit_4 = stats.norm.pdf(xcenter, loc = mean_4, scale = sigma_4) 

print('flux_4 :', mean_4, sigma_4) 
f.write('\nflux_4 ')
f.write(str(mean_4))
f.write(' ')
f.write(str(sigma_4))
...
ax.plot(xcenter, flux_fit_4/np.sum(flux_fit_4)*np.sum(hx), 'r--', markersize=20, label='normal distribution') 
ax.hist(flux_4, xgrid, color='k', alpha=0.5, label='original value')
ax.set_title('Bkg-distribution with four times clipping', fontsize=25)
legend = plt.legend(fontsize='xx-large')
plt.savefig('fitting_image_new')

# <------ Step 4. Comparison the different type of aperture ------>
...
pos_cut1[:,0] = positions_filter[:,0]-10000  # The coordinate of  apertures must follow the real frame of image
pos_cut1[:,1] = positions_filter[:,1]-10000
pos_cut2[:,0] = pos_obj[:,0]-10000
pos_cut2[:,1] = pos_obj[:,1]-10000
...
plt.imshow(bkg_data[10000:10500,10000:10500], norm=norm, origin='lower', cmap='gray_r')
